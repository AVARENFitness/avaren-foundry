...
export default function CoachScreen({ workspace, setWorkspace, screen='clients', program, selectedClient, setSelectedClient }) {
  ...
  const unassign=async(assignment)=>{if(!confirm(`Cancel ${assignment.title}? It will leave active schedules but remain in assignment history.`))return;try{await coachBackend.cancelAssignment(assignment.id);setNotice('Assignment cancelled and removed from active schedules.');await load()}catch(e){setNotice(e.message)}}
  const deleteAssignment=async(assignment)=>{if(assignment.status==='completed'){setNotice('Completed workouts cannot be deleted. Archive them instead.');return}if(!confirm(`Permanently delete ${assignment.title}? This removes it from Coach Hub, Calendar, the athlete account, and notifications.`))return;try{await coachBackend.deleteAssignment(assignment.id);setNotice('Assignment permanently deleted.');await load()}catch(e){setNotice(e.message)}}
